...
public class MeshImporter : Singleton<MeshImporter> {
    ...
    /// <param name="fileName">Asset file name (e.g., filename.obj, filename.fbx)</param>
    /// <param name="aoId">ID of action object which is asociated with mesh</param>
    /// <returns></returns>
    private IEnumerator DownloadMesh(string meshId, string fileName, string aoId) {

        // Build the mesh URL - assumes server provides /mesh/{meshId}/{fileName} endpoint
        // For example: ws://butcluster.ddns.net:6789/mesh/lab_mesh/mesh.obj
        Uri serverUri = CommunicationManager.Instance.ServerUri;
        string meshUrl = $"http://{serverUri.Host}:{serverUri.Port}/mesh/{meshId}/{fileName}";
        
        Debug.LogError($"Downloading mesh from: {meshUrl}");

        using (UnityWebRequest www = UnityWebRequest.Get(meshUrl)) {
            // Request and wait for the desired page.
            yield return www.SendWebRequest();
            if (www.result == UnityWebRequest.Result.ConnectionError || www.result == UnityWebRequest.Result.ProtocolError) {
                Debug.LogError($"Failed to download mesh {www.error}");
                ...
                if (Path.GetExtension(savePath).ToLower() == ".zip") {
                    ...
                    try {
                        ...
                        Debug.LogError(ex);
                        Debug.LogError("Failed to extract mesh");
                    }
                } else { //not *.zip
    ...
    private void OnModelLoadError(IContextualizedError obj) {
        Debug.LogError($"Unable to show mesh: {obj.GetInnerException().Message}");
    ...
    /// </summary>
    /// <param name="meshId"></param>
    /// <param name="fileName">File name of the mesh asset</param>
    /// <returns></returns>
    public bool CheckIfNewerRobotModelExists(string meshId, string fileName) {
        ...
        if (meshSources.TryGetValue(fileName, out bool downloadInProgress)) {
            if (downloadInProgress) {
                // download is in progress, return false so the mesh file won't download again
                return false;
            } else {
            ...
            // Create the entry in meshSources and set downloadProgress to true and start downloading
